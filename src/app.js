...
freezer.on("champion:choose", (champion) => {
  ...
  plugins[state.tab.active].getPages(champion, (res) => {
    ...
    if (freezer.get().plugins.remote[plugin]) freezer.get().plugins.remote[plugin].cache.set(champion, res);
  });
});
...
freezer.on("page:unlinkbookmark", (champion, page) => {
  if (freezer.get().lastbookmarkedpage.champion == champion && freezer.get().lastbookmarkedpage.page == page) freezer.get().lastbookmarkedpage.set({ page: null, champion: null });
  var state = freezer.get();
