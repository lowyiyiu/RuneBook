...
function createWindow() {
  ...
  const isOnADisplay = electron.screen
    .getAllDisplays()
    .map((display) => mainWindowState.x >= display.bounds.x && mainWindowState.x <= display.bounds.x + display.bounds.width && mainWindowState.y >= display.bounds.y && mainWindowState.y <= display.bounds.y + display.bounds.height)
...
app.on("ready", function () {
  ...
  win.webContents.on("did-finish-load", () => {
    ...
    request(
      {
        url: "https://api.github.com/repos/lowyiyiu/RuneBook/releases/latest",
        headers: {
          "User-Agent": "request",
        },
      },
